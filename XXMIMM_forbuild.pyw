...
def get_base_path():
    """Get the base path for accessing files, works for both development and bundled exe"""
...
SETTINGS_FILE = os.path.join(BASE_PATH, "mod_preferences.json")
MODS_FOLDER_FILE = os.path.join(BASE_PATH, "mods_folder_paths.json")
...
class ModManager(QWidget):
    ...
    def apply_styles(self):
        """Apply Windows 11 dark mode styles."""
        self.setStyleSheet("""
        ...
        # Set properties for special buttons
    ...
    def preview_mod(self, mod_folder):
        ...
        if os.path.exists(preview_path):
            try:
                ...
                QMessageBox.warning(self, "Error", f"Couldn't open preview:\n{e}")
        else:
            QMessageBox.information(self, "No Preview", f"No preview image found for {mod_folder}")
